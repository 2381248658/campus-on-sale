...
router.post("/", async (req, res) => {
  try {
    ...
    // 1. 基础参数校验
    if (!skuId) return res.status(400).json({ code: "400", msg: "缺少skuId" });
    if (!Number.isInteger(count) || count <= 0) {
      return res.status(400).json({ code: "400", msg: "商品数量必须为正整数" });
    }

    // 2. 校验 SKU 是否真实存在 (防止脏数据/幽灵数据)
    const skuExists = await Goods.findOne({ "skus.id": skuId }).lean();
    if (!skuExists) {
      return res.status(400).json({ code: "400", msg: "商品或规格不存在" });
    }
...
router.post("/merge", async (req, res) => {
  try {
    ...
    const validSkuIds = localCartList
      .filter(
        (item) => item.skuId && Number.isInteger(item.count) && item.count > 0,
      )
      .map((item) => item.skuId);
...
// 5.6 全选/取消全选 (必须放在 /:skuId 之前，否则 'selected' 会被当作 skuId 参数)
router.put("/selected", async (req, res) => {
  try {
    ...
    if (typeof selected !== "boolean") {
      return res.status(400).json({ code: "400", msg: "缺少selected参数" });
    }
...
router.put("/:skuId", async (req, res) => {
  try {
    ...
    if (Object.keys(updateData).length === 0) {
      return res.status(400).json({ code: "400", msg: "没有需要更新的数据" });
    }
